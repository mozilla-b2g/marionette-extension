...
const MARIONETTE_CONTRACTID = "@mozilla.org/marionetteextension;1";
...
MarionetteExtensionComponent.prototype.onSocketAccepted = function(
    socket, transport) {
  this.logger.info("onSocketAccepted for Marionette dummy socket");
};

MarionetteExtensionComponent.prototype.onStopListening = function(socket, status) {
  this.logger.info(`onStopListening for Marionette dummy socket, code ${status}`);
  socket.close();
};

/** Check cmdLine argument for {@code --marionette}. */
MarionetteExtensionComponent.prototype.handle = function(cmdLine) {
...
MarionetteExtensionComponent.prototype.observe = function(subj, topic, data) {
  switch (topic) {
...
MarionetteExtensionComponent.prototype.suppressSafeModeDialog_ = function(win) {
...
MarionetteExtensionComponent.prototype.init = function() {
...
MarionetteExtensionComponent.prototype.uninit = function() {
  if (!this.loaded_) {
    return;
  }
  this.server.stop();
